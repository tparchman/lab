...
---
---
title: "Sphaeralcea dataset: updated"
author: "COM"
date: 
output: html_document
code_download: true
...
**Table 1.** Genetic diversities
```{r echo=FALSE, message=FALSE, warning=FALSE,out.width='20%', fig.align="center"}
library(filenamer)
library(knitr)
library(kableExtra) 
library(dplyr)                                                         
data=read.csv(file = "/Users/carolinaosunamascaro/Google Drive/Mi unidad/Ahora/Sph_ordenado/Strong Filtering/Rmd/diversities.csv", sep=";")
kable(data, caption = "") %>%
...
<button class="btn btn-link" data-toggle="collapse" data-target="#fst">fst</button>
...
Fst_results <- read.csv("/Users/carolinaosunamascaro/Google Drive/Mi unidad/Ahora/Sph_ordenado/Strong Filtering/Rmd/Fst.csv")
d_btn <- Fst_results %>% 
  download_this(
...
library(kableExtra) 
library(dplyr)                                                         
data=read.csv(file = "/Users/carolinaosunamascaro/Google Drive/Mi unidad/Ahora/Sph_ordenado/Strong Filtering/Rmd/Components_covariance.csv", sep = ";")
kable(data, caption = "") %>%
  kable_styling(font_size = 15) %>%
  row_spec(0, bold = TRUE, italic = FALSE) %>% 
  #row_spec(7, underline = TRUE, monospace = TRUE) %>% 
...
![](PCA_UMAP/pca_spp.png){#id .class width=100% height=100%}
**PCA pop**

![](PCA_UMAP/pca_pop.png){#id .class width=100% height=100%}

**UMAP species**
![](PCA_UMAP/UMAP/umap_spp.png){#id .class width=100% height=100%}


**UMAP population level**
![](PCA_UMAP/UMAP/umap_pop.png){#id .class width=100% height=100%}


**Mean PCA, Procrustes:**

<button class="btn btn-link" data-toggle="collapse" data-target="#pca2">pca code</button>
<div id="pca2" class="collapse">   
<br>
```{r eval=FALSE}
setwd("/sph_pca_mean/")
library(vegan)
...
<br>
**Mean PCA nd Procrustes**

![](PCA_mean/mean_procrustes.png){#id .class width=100% height=100%}



### ADMIXTURE, piecharts

<button class="btn btn-link" data-toggle="collapse" data-target="#Admixture">admixture code</button>
<div id="Admixture" class="collapse">   
<br>
```{r eval=FALSE}
...
<br>
**Admixture K10**
![](admixture/k10.png){#id .class width=100% height=100%}

**Admixture piecharts:**

<button class="btn btn-link" data-toggle="collapse" data-target="#pie">pie code</button>
...
<br>
![](admixture/PIECHARTS/map_pie.png){#id .class width=100% height=100%}

**Individual admixture proportions:**

```{r echo=FALSE, message=FALSE, warning=FALSE, fig.align="right"}
library("EBImage")
setwd("/Users/carolinaosunamascaro/Google Drive/Mi unidad/Ahora/Sph_ordenado/Strong Filtering/admixture/PIECHARTS/individual/")
net_name <- list.files(path="/Users/carolinaosunamascaro/Google Drive/Mi unidad/Ahora/Sph_ordenado/Strong Filtering/admixture/PIECHARTS/individual/", pattern = "png")
display(readImage(net_name, "PNG"), method="browser")
```
...
**IBD**
<button class="btn btn-link" data-toggle="collapse" data-target="#code3">ibd code</button>
...
![](IBD/IBD.png){#id .class width=50% height=50%}


### Phylogeny 
<br>

**IQtree using the [Roostrap](http://www.iqtree.org/doc/Rootstrap) method for inferring trees without outgroup [(the paper here)](https://www.biorxiv.org/content/10.1101/2020.07.31.230144v2.full)**
<br>
Note: I’m showing here a phylogeny with a subset of one individual per population (the main phylogeny with all the individuals it’s too big for including it here, but the patterns are the same- no species clustering).
...
![](Phylogeny-ypyrad/tree_pop.png){#id .class width=100% height=100%}
